...
function changeOpacity() {
  tools[tools_keys[erase_key]].color = tools[
    tools_keys[erase_key]
  ].color.replace(current_opacity, $("#eraseropacity").val() * 1);
...
function toggleGrid(e) {
  ...
  var x =
    (pos.x + (grid_block_size - (pos.x % grid_block_size))) / grid_block_size -
    1;
  var y =
    (pos.y + (grid_block_size - (pos.y % grid_block_size))) / grid_block_size -
    1;
...
function drawMarkers() {
  for (var i = 0; i < markers.length; i++) {
    ...
    context.fillStyle = tools[markers[i][2]].color.replace(
      /,(\d+\.?\d*)\)/g,
      ",1)"
    );
      ...
      reader.onload = function (e) {
        if (e.target.readyState == FileReader.DONE) {
          ...
          background_img.onload = function () {
            ...
            if (
              background_img.width >
                parseInt($("#container").css("max-width"), 10) ||
              background_img.height >
                parseInt($("#container").css("max-height"), 10)
            ) {
...
function savePercent() {
  ...
  $("#savepercent").attr("href", "data:" + encodedData);
  $("#virtual").hide();
  return false;
}

function makeVirtual(mutablePercentData = {}) {
  ...
  virtual_context.fillRect(
    0,
    0,
    parseInt($("#virtual").attr("width"), 10),
    parseInt($("#virtual").attr("height"), 10)
  );
  ...
  var percentages = mutablePercentData;
  ...
  for (i = 0; i < tools_length - 1; i++) {
    // do not include last tool - the eraser
    if (percentages[tools_keys[i]]) {
      ...
      if (tools[tools_keys[i]].markers && marker_counts[tools_keys[i]]) {
        percentage_string =
          percentage_string + ", " + marker_counts[tools_keys[i]] + " counted";
      }
      var percentage_string = percentage_string + " ";
      var text_width = virtual_context.measureText(
        tools[tools_keys[i]].desc + percentage_string
      ).width;
      if (x + text_width > parseInt($("#canvas").attr("width"), 10)) {
        y = y + 21;
        x = 18;
      }
      virtual_context.fillStyle = tools[tools_keys[i]].color.replace(
        current_opacity,
        "1"
      );
      virtual_context.fillText(tools[tools_keys[i]].desc[0], x, y);
      virtual_context.fillStyle = "black";
      var first_char_width = virtual_context.measureText(
        tools[tools_keys[i]].desc[0]
      ).width;
      virtual_context.fillText(
        tools[tools_keys[i]].desc.substr(1) + percentage_string,
        x + first_char_width,
        y
      );
      x = x + text_width + 18;
  ...
  branding_x =
    parseInt($("#virtual").attr("width"), 10) -
    10 -
    virtual_context.measureText(branding).width;
  ...
  virtual_context.fillRect(
    branding_x - 12,
    branding_y - 18,
    virtual_context.measureText(branding).width - 13,
    27
  );
